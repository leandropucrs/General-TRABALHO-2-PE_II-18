...
void bemvindo(){
 ...
 case 3:{
    string linha;
  ifstream consulta ("historico.txt"); 
  if (consulta.is_open())
  {
    while (! consulta.eof() ) //enquanto fim do arquivo for falso, continua
    {
      getline (consulta,linha); // Pega cada linha
...
Rank(string tabela, string nome_jog, int MAX_JOG){
this->tabela = tabela;
this->nome_jog=nome_jog;
this->MAX_JOG=MAX_JOG;
}
  ...
  void gravar(vector<Rank>Ranks){
    ofstream historico;
        
    historico.open("historico.txt",std::ofstream::out | std::ofstream::app);
    if (historico.is_open())
  {
    historico << "Pontuação da partida" << endl;
        for ( int i = 0; i<Ranks.size(); ++i) {
		historico << i + 1 << ") ";
		historico << Ranks[i].tabela << "   "<<Ranks[i].nome_jog<<Ranks[i].MAX_JOG;
    		} 
        historico << endl;	
        historico << endl;
...
void Rank(vector <vector<int>> &tabela, vector<string> &get_nome, int MAX_JOG){
	...
	// Mostra vetor Pontuação
	cout << "Pontuação do Jogador: " << endl;
...
int main(){
	...
	do {
		...
		cin >> op;
		if (toupper(op) == 'S'){
      			//Mostra numeros de partidas
			for (int i = 0; i < turno + 1; ++i){
				cout << "\t Partida " << turno + 1;
			} cout << endl;
